...
#### `platform-xp-eks` repo → `https://github.com/urukube/platform-xp-eks`
Crossplane XRD + Composition for provisioning BU EKS clusters. Auto-discovered via `platform-custom-xrds` GitHub topic, synced by ArgoCD Application `xrd-platform-xp-eks`.

- Claim kind: `UEks` — composite kind: `XUEks` — group: `compute.platform.urukube.io/v1alpha1`
- XRD/Composition name: `xueks.compute.platform.urukube.io`
- Required parameters: `awsAccountId`, `region`, `buId`, `env`, `friendlyName`, `vpcId`, `subnetIds`, `subnetCidrs`, `nodeSecurityGroupId`, `controlPlaneSecurityGroupId`
- Optional: `kubernetesVersion` (default `"1.32"`), `nodeInstanceType` (default `t3.medium`), `minSize/maxSize/desiredSize` (2/3/2), `nodeAmiId`, `tags`, etc.
- Cluster name pattern: `{friendlyName}-eks` (external-name on EKS Cluster managed resource)
- **Composition uses `function-go-templating` (Pipeline mode)** — three pipeline steps: `resolve-environment` (EnvironmentConfigs), `patch-and-transform` (ProviderConfig via cross-account role chain), `generate-resources` (all EKS managed resources via go-template)
- Managed resources created: node IAM Role + InstanceProfile + 5× RPAs, cluster IAM Role + 1× RPA, EKS Cluster, kube-proxy Addon, LaunchTemplate (AL2023 nodeadm userdata), AutoScalingGroup, SecurityGroupIngressRule per subnet CIDR
- **Critical pattern for go-template nil guards**: Always use `with` blocks at every level of nested `atProvider` access — `vpcConfig.clusterSecurityGroupId`, `certificateAuthority.data`, `identity.oidc.issuer` are all nil until resources are Ready. Direct access like `$c.status.atProvider.identity.oidc.issuer` panics when `identity` is nil. `| default ""` does NOT protect against nil map access (only against nil values); use `{{- with .identity }}` before accessing `.oidc`
- **Status block guard**: Only write `XUEks status:` block `{{- if $clusterEndpoint }}` — writing it with empty fields renders YAML nulls that fail CRD validation
- **Composition updates**: ArgoCD controls this composition — `kubectl apply/replace` is reverted immediately. Always commit+push to Git and verify `kubectl get application xrd-platform-xp-eks -n argocd -o jsonpath='{.status.sync.revision}'` matches the new commit
- **Stuck-claim cleanup**: If a claim is deleted while EKS is CREATING, managed resources get stuck with "create operation is still running". Fix: delete the EKS cluster in AWS (`aws eks delete-cluster --name <name>`), then `kubectl patch <resource> -p '{"metadata":{"finalizers":[]}}' --type=merge` to remove finalizers
- **`certificateAuthority` and `identity.oidc` are LISTS in atProvider**: Upbound EKS v1beta2 returns `atProvider.certificateAuthority` and `atProvider.identity` as `[]interface{}`. Use `index . 0` not `with .certificateAuthority`: `{{- with index .certificateAuthority 0 }}{{- $caData = .data | default "" }}{{- end }}` and `{{- with index .identity 0 }}{{- with index .oidc 0 }}...{{- end }}{{- end }}`. `vpcConfig` is a map (not a list).
- **nodeAmiId required for ASG**: Self-managed nodes with custom launch templates require an explicit `imageId`. Get via: `aws ssm get-parameter --name "/aws/service/eks/optimized-ami/<version>/amazon-linux-2023/x86_64/standard/recommended/image_id" --profile terraform-check --region us-east-1 --query Parameter.Value --output text`. Set in claim as `nodeAmiId`. If ASG hits `AsyncCreateFailure`, delete the ASG managed resource to force recreation after fixing the LT.
- Example claim: `C:\idp\platform-xp-example-resources\eks\claim-eks-minimal.yaml` (BU001/dev, 1.36, nodeAmiId ami-082b37985d4f2aa44, uses UNetwork outputs for VPC/subnet IDs)
...
- Platform services are exposed at `platform.urukube.io` via path-based Istio VirtualServices: `/argocd` (ArgoCD), `/kiali` (Kiali), `/prometheus` (Prometheus). Add `<NLB-IP> platform.urukube.io` to hosts file for local access. NLB DNS: `k8s-istiosys-istioing-7eec224ec5-be1494c3f2515029.elb.us-east-1.amazonaws.com` (NLB DNS changes on each cluster rebuild — always resolve fresh with `kubectl get svc -n istio-system istio-ingressgateway`)
...
- **Post-cluster-rebuild required step**: When the orchestrator cluster is rebuilt, `orchestrator-eks-crossplane` is recreated with a new IAM role ID. The trust policy on `urukube-crossplane-role` stores the role ID (not ARN), so it goes stale and Crossplane claims fail with `AccessDenied: not authorized to perform sts:AssumeRole`. Fix: `aws iam update-assume-role-policy --role-name urukube-crossplane-role --policy-document '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"AWS":"arn:aws:iam::485646919618:role/orchestrator-eks-crossplane"},"Action":"sts:AssumeRole"}]}'` — always use the role ARN (not ID) so it survives rebuilds.
