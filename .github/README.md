...
A single **Orchestrator Cluster** (PROD + DR) runs Crossplane, ArgoCD, and Backstage. It holds no BU workloads — its sole job is to provision and manage infrastructure for every customer on demand via CRDs.

> Full topology diagram, component breakdown, BU provisioning flow, and DR strategy: [CLUSTER-TOPOLOGY.md](CLUSTER-TOPOLOGY.md)
